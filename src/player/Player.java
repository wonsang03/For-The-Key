...
public class Player extends Entity {
    ...
    public int hp = 100;
    public double attackMultiplier = 1.0;     // private -> public 으로 변경
    ...
    public void update() {
        ...
        if (keyH.upPressed) { moveY -= speed; isMoving = true; }
        if (keyH.downPressed) { moveY += speed; isMoving = true; }
        if (keyH.leftPressed) { moveX -= speed; isMoving = true; }
        if (keyH.rightPressed) { moveX += speed; isMoving = true; }
        ...
        if (isMoving) {
            ...
            if (footstepCounter > 20) { // 약 0.3초마다 재생
                gp.sound.playSE(19);    // 19번: player_move.wav
                footstepCounter = 0;    
            }
        } else {
            footstepCounter = 20; 
    ...
    public void draw(Graphics2D g2) {
        ...
        // ★ [핵심] 무적 상태일 때 빨간색 틴트(Tint) 적용하기 (마인크래프트 효과)
        if (invincible == true) {
    ...
    public void addAttackBonus(double value) { attackMultiplier += value; }
    public void addSpeedBonus(double value) { baseSpeed += value; speed = (int)baseSpeed; }
    public void addAttackSpeedBonus(double value) { attackSpeedBonus += value; }
    public void addMaxHP(double value) { maxHP += (int)value; hp += (int)value; }
    ...
    public void receiveDamage(int damage) {
        // [민정 추가] 무적 상태라면 데미지 무시
        if (invincible == true) {
            return;
        }

        // 피격 사운드 재생 (20번: player_hit.wav)
        gp.sound.playSE(20);
        ...
        if (this.hp <= 0) {
            this.hp = 0;
            // gp.sound.playSE(21); // (필요 시 주석 해제) 플레이어 사망음
            System.out.println("플레이어 사망!");
        }
    }
    
    // (사용 안 함 - GamePanel의 SoundManager 사용 권장)
    public void playSound(String soundName) {
        try {
            ...
            System.out.println("소리 재생 오류: " + e.getMessage());
        }
    }
}