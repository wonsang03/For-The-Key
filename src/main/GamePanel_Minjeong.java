...
import system.SoundManager;  //민정 추가
...
public class GamePanel_minjeong extends JPanel implements Runnable, KeyListener, MouseMotionListener, MouseListener { 
    ...
    // ★ [사운드 시스템]
    public SoundManager sound = new SoundManager(); // 민정 추가

    // [김민정님 코드] 플레이어 및 KeyHandler
    public KeyHandler keyH = new KeyHandler(this); //민정 수정: private - > public
    public Player player = new Player(this, keyH); // 민정 수정 (public Player player;에서 변경함)
    ...
    public GamePanel_minjeong() {
        ...
        this.setFocusTraversalKeysEnabled(false);  //민정 추가 (Tab 누를 시 정보창 띄우기 위함)

        // [추가] 
        this.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mousePressed(java.awt.event.MouseEvent e) {
                // 마우스가 클릭되면 UI에게 확인하라고 시킵니다.
                if (ui != null) {
    ...
    public void setupGame() {
        ...
        // ★ [게임 시작 시 타이틀 음악 재생]
        sound.playMusic(29); // 민정 추가: 타이틀 화면 BGM (29번) 재생
    ...
    public void update() {
        ...
        for (Enemy enemy : enemies) {
            if (enemy.isDead()) {
                ...
                // ★ [적 사망 사운드]
                sound.playSE(28); // 민정 추가: 적 사망 효과음
            }
        }
        ...
        if (player.getHP() <= 0) {
            ...
            sound.stop();     // 민정 추가: 배경음악 정지
            sound.playSE(21); // 민정 추가: 플레이어 사망(게임오버) 효과음
        }
    }
        ...
        if (direction != null && currentRoom.hasConnection(direction)) {
            ...
            if (targetRoomId != null) {
                ...
                if (nextRoom != null) {
                    ...
                    // ★ [방 이동 사운드 처리]
                    playStageMusic(); // 민정 추가: 스테이지에 맞는 배경음악 체크/재생
                    sound.playSE(18); // 민정 추가: 문 열리는 소리
                }
            }
        }
    }
    ...
    private void shoot() {
        ...
        switch (currentWeapon) {
            // 근접 무기 (검)
        	case DAGGER: sound.playSE(0); break; // 민정 추가: 단검 소리
            case KNIGHT_SWORD: sound.playSE(1); break; // 민정 추가: 롱소드 소리
            case LONG_SWORD: sound.playSE(2); break; // 민정 추가: 대검 소리
            
            // 원거리 무기 (총)
            case PISTOL: sound.playSE(3); break; // 민정 추가: 권총 소리
            case SHOTGUN: sound.playSE(4); break; // 민정 추가: 샷건 소리
            case SNIPER: sound.playSE(5); break; // 민정 추가: 스나이퍼 소리
            default: sound.playSE(3); break; 
        }
    ...
    private void checkItemPickups() {
        ...
        for (Item item : items) {
            if (!item.isPicked() && playerRect.intersects(item.getBounds())) {
                ...
                // ★ [아이템 획득 사운드]
                sound.playSE(15); // 민정 추가: 아이템 획득 소리
            }
        }
    }
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (gameState == titleState) {
            if (code == KeyEvent.VK_ENTER) {
                ...
                // ★ [타이틀 -> 게임 시작 사운드 전환]
                sound.stop();      // 민정 추가: 타이틀 음악(29번) 정지
                playStageMusic();  // 민정 추가: 스테이지 배경음(6번) 시작
            }
        }
        // 2. 플레이 중
    ...
    // ★ [사운드 재생 헬퍼 메소드]
    public void playStageMusic() { // 민정 추가: 스테이지별 음악 자동 재생 메소드
        int currentStage = MapLoader.getCurrentStage();
